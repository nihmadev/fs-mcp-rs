...
export function DashboardSettings(props: SettingsProps) {
  const { editor } = props;
  const [activeTab, setActiveTab] = useState<"general" | "access" | "limits">("general");
  const actions = useProfileActions(editor);
  ...
  return (
    <div className="dashboard-page settings-page screen-enter">
      ...
      {activeTab === "general" && <div id="settings-panel-general" className="settings-tab-panel" role="tabpanel" aria-labelledby="settings-tab-general">
        <ProfileSection editor={editor} actions={actions} />
        <GeneralSection editor={editor} setValue={setValue} />
        <button className="outlined-button setup-again" type="button" onClick={props.onOpenSetup}><Icon name="restart_alt" /> Open setup wizard</button>
      </div>}
